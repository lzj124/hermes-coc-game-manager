...
description: COC TRPG 跑团主控台 — 唯一入口。管理角色、线索、场景推进、骰子检定、跨会话记忆、多战役隔离。所有跑团相关请求只加载此 skill，无需加载 coc-dice（内部自动调用）。不存在 "coc-keeper" skill，不要加载或创建。
triggers: 跑团, COC, 克苏鲁, 守秘人, KP, 创建角色, 调查员, 车卡, 剧本大纲, 开始游戏, 继续跑团, 上次跑到哪了, 检定, SAN, HP, NPC, 线索, 道具, 模组, 剧本, 新剧本, scenario
---

# COC 游戏管理器 — 跑团唯一入口

> ⚠️ **这是 COC 跑团唯一需要加载的 skill。** 加载这个就够，无需 coc-dice（自动调用其 roll.py）。
...
玩家说"完团了"、"结束了"、或剧情推进到结局节点后，agent 必须主动审查流程。

**三步必须做：**

1. **读状态** → `game.py resume` 获取最终状态
2. **清理记忆** → `game.py note event "已解决: ..."` 清掉 campaign-memory.md 里所有"已触发但未解决"的事件
3. **补记偏好** → 如果 KP 偏好栏为空，补记本战役观察到的玩家风格
...
| `references/post-campaign-review.md` | **结案审查清单 — 完团后必读** |
